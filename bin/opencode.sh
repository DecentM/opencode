...
if ! which gns >/dev/null 2>/dev/null; then
    echo "gns-cli not installed, not available via \$PATH, or the installation is broken. Please (re)install gns client and try again."
    exit 1
fi
...
# Install themes (opencode does not check $OPENCODE_CONFIG_DIR for themes)
...
if [ ! -f "$OPENCODE_CONFIG_DIR/.env" ]; then
    ...
    printf "\n"
    printf "You should now establish an alias for '%s' to run OpenCode with the Chesscom configuration." "$SELF_PATH"
    printf "\n"
...
DOCKER_BUILDKIT=1 docker build -f "$DOCKER_DIR/mcp-flaresolverr.dockerfile" \
    --cache-from ghcr.io/chesscom/opencode-flaresolverr:latest \
    -t chess-opencode-flaresolverr "$DOCKER_DIR" 2>/dev/null &

DOCKER_BUILDKIT=1 docker build -f "$DOCKER_DIR/mcp-playwright.dockerfile" \
    --cache-from ghcr.io/chesscom/opencode-playwright:latest \
    -t chess-opencode-playwright "$DOCKER_DIR" 2>/dev/null &

DOCKER_BUILDKIT=1 docker build -f "$DOCKER_DIR/mcp-figma.dockerfile" \
    --cache-from ghcr.io/chesscom/opencode-figma:latest \
    -t chess-opencode-figma "$DOCKER_DIR" 2>/dev/null &

DOCKER_BUILDKIT=1 docker build -f "$DOCKER_DIR/tool-node.dockerfile" \
    --cache-from ghcr.io/chesscom/opencode-node-sandbox:latest \
    -t chess-opencode-node-sandbox "$DOCKER_DIR" 2>/dev/null &

DOCKER_BUILDKIT=1 docker build -f "$DOCKER_DIR/tool-python.dockerfile" \
    --cache-from ghcr.io/chesscom/opencode-python-sandbox:latest \
    -t chess-opencode-python-sandbox "$DOCKER_DIR" 2>/dev/null &

DOCKER_BUILDKIT=1 docker build -f "$DOCKER_DIR/tool-tesseract.dockerfile" \
    --cache-from ghcr.io/chesscom/opencode-tesseract:latest \
    -t chess-opencode-tesseract "$DOCKER_DIR" 2>/dev/null &

wait

for image in \
    ghcr.io/chesscom/opencode-flaresolverr:latest \
    ghcr.io/chesscom/opencode-playwright:latest \
    ghcr.io/chesscom/opencode-figma:latest \
    ghcr.io/chesscom/opencode-node-sandbox:latest \
    ghcr.io/chesscom/opencode-python-sandbox:latest \
    ghcr.io/chesscom/opencode-tesseract:latest; do
