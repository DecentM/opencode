...
| Content write | **communicator** | Only agent with full MCP write access |
...
> **Note**: The personal-assistant agent uses Playwright for browser automation. It can book restaurants, flights, hotels, appointments; shop and compare prices (confirms with user before purchases); research and gather information from any website; manage accounts and subscriptions; and handle Workspace (Mail, Calendar, Drive). First use requires manual login for authenticated sites. Subsequent sessions use persistent cookies.
